...
### 3. Smart Tap & Manual Mode
- **Smart Tap (기본)**: `I` 키만 누르면 자동으로 현재 라인의 시작을 기록하고, 이전 라인을 종료하며, 문장 길이에 맞춰 종료 시간을 자동 계산합니다.
- **Manual 2-Tap**: 느린 곡이나 정밀한 타이밍을 위해 `I` (시작) → `O` (종료)를 직접 입력하는 모드를 지원합니다.
- **End Lock**: `E` 키를 눌러 현재 라인의 종료 시간을 수동으로 고정(Lock)하고 다음으로 넘어갑니다.
- **Visual Timeline**: 줌 가능한 오디오 파형과 자막 블록을 시각적으로 확인하며 작업합니다.
...
- **Premiere Pro XML**: 어도비 프리미어 프로 (FCP7 XML) 호환 마커
- **Final Cut Pro XML**: 파이널 컷 프로 X (FCPXML) 호환 마커
