    ...
    return (
      <div className="w-full">
        ...
        <div className="mb-4 flex items-center gap-3">
          <span className={`text-sm font-medium transition-colors ${mode === 'file' ? 'text-gray-700' : 'text-gray-400'
            }`}>
            파일 업로드
          </span>
          <button
          ...
          >
            <span
              className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${mode === 'file' ? 'translate-x-6' : 'translate-x-1'
                }`}
            />
          </button>
          <span className={`text-sm font-medium transition-colors ${mode === 'timer' ? 'text-gray-700' : 'text-gray-400'
            }`}>
            파일 없이 시작
          </span>
        </div>
